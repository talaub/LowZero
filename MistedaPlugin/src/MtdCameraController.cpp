...
namespace Mtd {
  namespace Component {
    ...
    Low::Util::Instances::Slot *CameraController::ms_Slots = 0;
    Low::Util::List<CameraController> CameraController::ms_LivingInstances =
        Low::Util::List<CameraController>();

    CameraController::CameraController() : Low::Util::Handle(0ull)
    {
    }
    CameraController::CameraController(uint64_t p_Id) : Low::Util::Handle(p_Id)
    {
    }
    CameraController::CameraController(CameraController &p_Copy)
    ...
    CameraController CameraController::make(Low::Core::Entity p_Entity)
    {
      ...
      CameraController l_Handle;
      l_Handle.m_Data.m_Index = l_Index;
      l_Handle.m_Data.m_Generation = ms_Slots[l_Index].m_Generation;
      l_Handle.m_Data.m_Type = CameraController::TYPE_ID;

      ACCESSOR_TYPE_SOA(l_Handle, CameraController, distance, float) = 0.0f;
      new (&ACCESSOR_TYPE_SOA(l_Handle, CameraController, entity,
    ...
    void CameraController::destroy()
    {
      ...
      const CameraController *l_Instances = living_instances();
    ...
    void CameraController::initialize()
    {
      ...
      LOW_PROFILE_ALLOC(type_buffer_CameraController);
      LOW_PROFILE_ALLOC(type_slots_CameraController);
      ...
      l_TypeInfo.serialize = &CameraController::serialize;
      ...
      l_TypeInfo.component = true;
      {
        ...
        l_PropertyInfo.dataOffset = offsetof(CameraControllerData, distance);
        ...
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
          ...
          CameraController l_Handle = p_Handle.get_id();
          l_Handle.set_distance(*(float *)p_Data);
        };
      ...
      }
      {
        ...
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle) -> void const * {
          CameraController l_Handle = p_Handle.get_id();
          l_Handle.get_entity();
          return (void *)&ACCESSOR_TYPE_SOA(p_Handle, CameraController, entity,
                                            Low::Core::Entity);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {
      ...
      }
      {
        ...
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset = offsetof(CameraControllerData, unique_id);
    ...
    void CameraController::cleanup()
    {
      ...
      LOW_PROFILE_FREE(type_buffer_CameraController);
      LOW_PROFILE_FREE(type_slots_CameraController);
    }

    CameraController CameraController::find_by_index(uint32_t p_Index)
    {
    ...
    bool CameraController::is_alive() const
    {
      return m_Data.m_Type == CameraController::TYPE_ID &&
             check_alive(ms_Slots, CameraController::get_capacity());
    }
    ...
    void CameraController::serialize(Low::Util::Handle p_Handle,
                                     Low::Util::Yaml::Node &p_Node)
    {
      CameraController l_CameraController = p_Handle.get_id();
      l_CameraController.serialize(p_Node);
    }

    Low::Util::Handle
    CameraController::deserialize(Low::Util::Yaml::Node &p_Node,
                                  Low::Util::Handle p_Creator)
    {
      CameraController l_Handle = CameraController::make(p_Creator.get_id());
      ...
      if (p_Node["distance"]) {
        l_Handle.set_distance(p_Node["distance"].as<float>());
      }
      if (p_Node["unique_id"]) {
    ...
    float CameraController::get_distance() const
    {
      ...
      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_distance
      // LOW_CODEGEN::END::CUSTOM:GETTER_distance

      return TYPE_SOA(CameraController, distance, float);
    }
    void CameraController::set_distance(float p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_distance
      // LOW_CODEGEN::END::CUSTOM:PRESETTER_distance

      // Set new value
      TYPE_SOA(CameraController, distance, float) = p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_distance
      // LOW_CODEGEN::END::CUSTOM:SETTER_distance
    }

    Low::Core::Entity CameraController::get_entity() const
    {
      _LOW_ASSERT(is_alive());
      ...
      return TYPE_SOA(CameraController, entity, Low::Core::Entity);
    }
    void CameraController::set_entity(Low::Core::Entity p_Value)
    {
      _LOW_ASSERT(is_alive());
      ...
      // Set new value
      TYPE_SOA(CameraController, entity, Low::Core::Entity) = p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_entity
      // LOW_CODEGEN::END::CUSTOM:SETTER_entity
    }

    Low::Util::UniqueId CameraController::get_unique_id() const
    {
      _LOW_ASSERT(is_alive());
    ...
    uint32_t CameraController::create_instance()
    {
      uint32_t l_Index = 0u;
    ...
    void CameraController::increase_budget()
    {
      ...
      uint8_t *l_NewBuffer = (uint8_t *)malloc(
          (l_Capacity + l_CapacityIncrease) * sizeof(CameraControllerData));
      ...
      {
        memcpy(
            &l_NewBuffer[offsetof(CameraControllerData, distance) *
                         (l_Capacity + l_CapacityIncrease)],
            &ms_Buffer[offsetof(CameraControllerData, distance) * (l_Capacity)],
            l_Capacity * sizeof(float));
      }
      {
        memcpy(
            &l_NewBuffer[offsetof(CameraControllerData, entity) *
                         (l_Capacity + l_CapacityIncrease)],
            &ms_Buffer[offsetof(CameraControllerData, entity) * (l_Capacity)],
