...
namespace Low {
  namespace Renderer {
    namespace Vulkan {
      ...
      bool DoubleBufferedDynamicBuffer::reserve(
          u32 p_ElementCount, u32 *p_StartOut)
      {
        if (m_FreeSlots.empty()) {
          *p_StartOut = 0;
          return false;
        ...
        *p_StartOut = l_FreeSlot.start;
        ...
        const uint32_t l_SavePoint = l_FreeSlot.start;
        if (l_FreeSlot.length == p_ElementCount) {
          m_FreeSlots.erase(m_FreeSlots.begin() + i_SlotIndex);
        } else {
          m_FreeSlots[i_SlotIndex].length =
      ...
      void DoubleBufferedDynamicBuffer::free(
          uint32_t p_Position, uint32_t p_ElementCount)
      {
        uint32_t l_ClosestUnder = ~0u;
        uint32_t l_ClosestOver = ~0u;
        uint32_t l_UnderDiff = ~0u;
        ...
        for (uint32_t i = 0u; i < m_FreeSlots.size(); ++i) {
          ...
          if (i_Slot.start < p_Position) {
            LOW_ASSERT((i_Slot.start + i_Slot.length) <= p_Position,
                       "Tried to double free from double buffered "
                       "dynamic buffer");
      ...
      void DoubleBufferedDynamicBuffer::clear()
      {
        m_FreeSlots.clear();
        m_FreeSlots.push_back({0, m_ElementCount});
      }

      void DoubleBufferedDynamicBuffer::destroy()
      {
        clear();
        BufferUtil::destroy_buffer(m_Buffers[0]);
        BufferUtil::destroy_buffer(m_Buffers[1]);
      }

      void DoubleBufferedDynamicBuffer::swap()
      {
        m_CurrentBufferIndex = 1u - m_CurrentBufferIndex;
      }

      AllocatedBuffer &DoubleBufferedDynamicBuffer::current()
      {
        return m_Buffers[m_CurrentBufferIndex];
      }

      AllocatedBuffer &DoubleBufferedDynamicBuffer::previous()
      {
        return m_Buffers[1u - m_CurrentBufferIndex];
      }

      u32 DoubleBufferedDynamicBuffer::get_current_buffer_index() const
      {
        return m_CurrentBufferIndex;
      }

      u32
      DoubleBufferedDynamicBuffer::get_previous_buffer_index() const
      {
        return 1u - m_CurrentBufferIndex;
      }

      uint32_t DoubleBufferedDynamicBuffer::get_used_elements() const
      {
        const uint32_t l_MaxElements = m_ElementCount;
        uint32_t l_FreeElements = 0;
