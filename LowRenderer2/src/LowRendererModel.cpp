#include "LowRendererModel.h"

#include <algorithm>

#include "LowUtil.h"
#include "LowUtilAssert.h"
#include "LowUtilLogger.h"
#include "LowUtilProfiler.h"
#include "LowUtilConfig.h"
#include "LowUtilHashing.h"
#include "LowUtilSerialization.h"
#include "LowUtilObserverManager.h"

// LOW_CODEGEN:BEGIN:CUSTOM:SOURCE_CODE

#include "LowRendererResourceManager.h"
// LOW_CODEGEN::END::CUSTOM:SOURCE_CODE

namespace Low {
  namespace Renderer {
    // LOW_CODEGEN:BEGIN:CUSTOM:NAMESPACE_CODE

    // LOW_CODEGEN::END::CUSTOM:NAMESPACE_CODE

    u16 Model::ms_TypeId = 0;
    const Low::Util::TypeIdentifier
        Model::IDENTIFIER(LOW_NAME(509652687), LOW_NAME(374627805));
    uint32_t Model::ms_Capacity = 0u;
    uint32_t Model::ms_PageSize = 0u;
    Low::Util::List<Model> Model::ms_LivingInstances;
    Low::Util::List<Low::Util::Instances::Page *> Model::ms_Pages;

    Low::Util::Handle Model::_make(Low::Util::Name p_Name)
    {
      return make(p_Name).get_id();
    }

    Model Model::make(Low::Util::Name p_Name)
    {
      return make(p_Name, 0ull);
    }

    Model Model::make(Low::Util::Name p_Name,
                      Low::Util::UniqueId p_UniqueId)
    {
      u32 l_PageIndex = 0;
      u32 l_SlotIndex = 0;
      uint32_t l_Index = create_instance(l_PageIndex, l_SlotIndex);

      Model l_Handle;
      l_Handle.m_Data.m_Index = l_Index;
      l_Handle.m_Data.m_Generation =
          ms_Pages[l_PageIndex]->slots[l_SlotIndex].m_Generation;
      l_Handle.m_Data.m_Type = Model::ms_TypeId;

      new (ACCESSOR_TYPE_SOA_PTR(l_Handle, Model, resource,
                                 Low::Renderer::ModelResource))
          Low::Renderer::ModelResource();
      new (ACCESSOR_TYPE_SOA_PTR(l_Handle, Model, lod0,
                                 Low::Renderer::Mesh))
          Low::Renderer::Mesh();
      new (ACCESSOR_TYPE_SOA_PTR(l_Handle, Model, lod1,
                                 Low::Renderer::Mesh))
          Low::Renderer::Mesh();
      new (ACCESSOR_TYPE_SOA_PTR(l_Handle, Model, lod2,
                                 Low::Renderer::Mesh))
          Low::Renderer::Mesh();
      new (ACCESSOR_TYPE_SOA_PTR(l_Handle, Model, lod3,
                                 Low::Renderer::Mesh))
          Low::Renderer::Mesh();
      ACCESSOR_TYPE_SOA(l_Handle, Model, name, Low::Util::Name) =
          Low::Util::Name(0u);

      l_Handle.set_name(p_Name);

      ms_LivingInstances.push_back(l_Handle);

      if (p_UniqueId > 0ull) {
        l_Handle.set_unique_id(p_UniqueId);
      } else {
        l_Handle.set_unique_id(
            Low::Util::generate_unique_id(l_Handle.get_id()));
      }
      Low::Util::register_unique_id(l_Handle.get_unique_id(),
                                    l_Handle.get_id());

      // LOW_CODEGEN:BEGIN:CUSTOM:MAKE

      ResourceManager::register_asset(l_Handle.get_unique_id(),
                                      l_Handle);
      // LOW_CODEGEN::END::CUSTOM:MAKE

      return l_Handle;
    }

    void Model::destroy()
    {
      LOW_ASSERT(is_alive(), "Cannot destroy dead object");

      {
        // LOW_CODEGEN:BEGIN:CUSTOM:DESTROY

        // LOW_CODEGEN::END::CUSTOM:DESTROY
      }

      broadcast_observable(OBSERVABLE_DESTROY);

      Low::Util::remove_unique_id(get_unique_id());

      u32 l_PageIndex = 0;
      u32 l_SlotIndex = 0;
      _LOW_ASSERT(
          get_page_for_index(get_index(), l_PageIndex, l_SlotIndex));
      Low::Util::Instances::Page *l_Page = ms_Pages[l_PageIndex];

      l_Page->slots[l_SlotIndex].m_Occupied = false;
      l_Page->slots[l_SlotIndex].m_Generation++;

      for (auto it = ms_LivingInstances.begin();
           it != ms_LivingInstances.end();) {
        if (it->get_id() == get_id()) {
          it = ms_LivingInstances.erase(it);
        } else {
          it++;
        }
      }
    }

    void Model::initialize()
    {
      const Low::Util::TypeIdentifier l_IdentifierNames(
          N(LowRenderer2), N(Model));

      // LOW_CODEGEN:BEGIN:CUSTOM:PREINITIALIZE

      // LOW_CODEGEN::END::CUSTOM:PREINITIALIZE

      ms_Capacity =
          Low::Util::Config::get_capacity(N(LowRenderer2), N(Model));

      ms_PageSize = Low::Math::Util::clamp(
          Low::Math::Util::next_power_of_two(ms_Capacity), 8, 32);
      {
        u32 l_Capacity = 0u;
        while (l_Capacity < ms_Capacity) {
          Low::Util::Instances::Page *i_Page =
              new Low::Util::Instances::Page;
          Low::Util::Instances::initialize_page(
              i_Page, Model::Data::get_size(), ms_PageSize);
          ms_Pages.push_back(i_Page);
          l_Capacity += ms_PageSize;
        }
        ms_Capacity = l_Capacity;
      }

      Low::Util::RTTI::TypeInfo l_TypeInfo;
      l_TypeInfo.name = N(Model);
      l_TypeInfo.typeId = ms_TypeId;
      l_TypeInfo.get_capacity = &get_capacity;
      l_TypeInfo.is_alive = &Model::is_alive;
      l_TypeInfo.destroy = &Model::destroy;
      l_TypeInfo.serialize = &Model::serialize;
      l_TypeInfo.deserialize = &Model::deserialize;
      l_TypeInfo.find_by_index = &Model::_find_by_index;
      l_TypeInfo.notify = &Model::_notify;
      l_TypeInfo.post_load = nullptr;
      l_TypeInfo.find_by_name = &Model::_find_by_name;
      l_TypeInfo.make_component = nullptr;
      l_TypeInfo.make_default = &Model::_make;
      l_TypeInfo.duplicate_default = &Model::_duplicate;
      l_TypeInfo.duplicate_component = nullptr;
      l_TypeInfo.get_living_instances =
          reinterpret_cast<Low::Util::RTTI::LivingInstancesGetter>(
              &Model::living_instances);
      l_TypeInfo.get_living_count = &Model::living_count;
      l_TypeInfo.component = false;
      l_TypeInfo.uiComponent = false;
      {
        // Property: resource
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
        l_PropertyInfo.name = N(resource);
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset = offsetof(Model::Data, resource);
        l_PropertyInfo.size = sizeof(Model::Data::resource);
        l_PropertyInfo.type = Low::Util::RTTI::PropertyType::HANDLE;
        l_PropertyInfo.handleType =
            Low::Renderer::ModelResource::IDENTIFIER;
        l_PropertyInfo.get_return =
            [](Low::Util::Handle p_Handle) -> void const * {
          Model l_Handle = p_Handle.get_id();
          l_Handle.get_resource();
          return (void *)&ACCESSOR_TYPE_SOA(
              p_Handle, Model, resource,
              Low::Renderer::ModelResource);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {
          Model l_Handle = p_Handle.get_id();
          l_Handle.set_resource(
              *(Low::Renderer::ModelResource *)p_Data);
        };
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle,
                                void *p_Data) {
          Model l_Handle = p_Handle.get_id();
          *((Low::Renderer::ModelResource *)p_Data) =
              l_Handle.get_resource();
        };
        l_TypeInfo.properties[l_PropertyInfo.name] = l_PropertyInfo;
        // End property: resource
      }
      {
        // Property: lod0
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
        l_PropertyInfo.name = N(lod0);
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset = offsetof(Model::Data, lod0);
        l_PropertyInfo.size = sizeof(Model::Data::lod0);
        l_PropertyInfo.type = Low::Util::RTTI::PropertyType::HANDLE;
        l_PropertyInfo.handleType = Low::Renderer::Mesh::IDENTIFIER;
        l_PropertyInfo.get_return =
            [](Low::Util::Handle p_Handle) -> void const * {
          Model l_Handle = p_Handle.get_id();
          l_Handle.get_lod0();
          return (void *)&ACCESSOR_TYPE_SOA(p_Handle, Model, lod0,
                                            Low::Renderer::Mesh);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {
          Model l_Handle = p_Handle.get_id();
          l_Handle.set_lod0(*(Low::Renderer::Mesh *)p_Data);
        };
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle,
                                void *p_Data) {
          Model l_Handle = p_Handle.get_id();
          *((Low::Renderer::Mesh *)p_Data) = l_Handle.get_lod0();
        };
        l_TypeInfo.properties[l_PropertyInfo.name] = l_PropertyInfo;
        // End property: lod0
      }
      {
        // Property: lod1
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
        l_PropertyInfo.name = N(lod1);
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset = offsetof(Model::Data, lod1);
        l_PropertyInfo.size = sizeof(Model::Data::lod1);
        l_PropertyInfo.type = Low::Util::RTTI::PropertyType::HANDLE;
        l_PropertyInfo.handleType = Low::Renderer::Mesh::IDENTIFIER;
        l_PropertyInfo.get_return =
            [](Low::Util::Handle p_Handle) -> void const * {
          Model l_Handle = p_Handle.get_id();
          l_Handle.get_lod1();
          return (void *)&ACCESSOR_TYPE_SOA(p_Handle, Model, lod1,
                                            Low::Renderer::Mesh);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {
          Model l_Handle = p_Handle.get_id();
          l_Handle.set_lod1(*(Low::Renderer::Mesh *)p_Data);
        };
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle,
                                void *p_Data) {
          Model l_Handle = p_Handle.get_id();
          *((Low::Renderer::Mesh *)p_Data) = l_Handle.get_lod1();
        };
        l_TypeInfo.properties[l_PropertyInfo.name] = l_PropertyInfo;
        // End property: lod1
      }
      {
        // Property: lod2
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
        l_PropertyInfo.name = N(lod2);
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset = offsetof(Model::Data, lod2);
        l_PropertyInfo.size = sizeof(Model::Data::lod2);
        l_PropertyInfo.type = Low::Util::RTTI::PropertyType::HANDLE;
        l_PropertyInfo.handleType = Low::Renderer::Mesh::IDENTIFIER;
        l_PropertyInfo.get_return =
            [](Low::Util::Handle p_Handle) -> void const * {
          Model l_Handle = p_Handle.get_id();
          l_Handle.get_lod2();
          return (void *)&ACCESSOR_TYPE_SOA(p_Handle, Model, lod2,
                                            Low::Renderer::Mesh);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {
          Model l_Handle = p_Handle.get_id();
          l_Handle.set_lod2(*(Low::Renderer::Mesh *)p_Data);
        };
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle,
                                void *p_Data) {
          Model l_Handle = p_Handle.get_id();
          *((Low::Renderer::Mesh *)p_Data) = l_Handle.get_lod2();
        };
        l_TypeInfo.properties[l_PropertyInfo.name] = l_PropertyInfo;
        // End property: lod2
      }
      {
        // Property: lod3
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
        l_PropertyInfo.name = N(lod3);
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset = offsetof(Model::Data, lod3);
        l_PropertyInfo.size = sizeof(Model::Data::lod3);
        l_PropertyInfo.type = Low::Util::RTTI::PropertyType::HANDLE;
        l_PropertyInfo.handleType = Low::Renderer::Mesh::IDENTIFIER;
        l_PropertyInfo.get_return =
            [](Low::Util::Handle p_Handle) -> void const * {
          Model l_Handle = p_Handle.get_id();
          l_Handle.get_lod3();
          return (void *)&ACCESSOR_TYPE_SOA(p_Handle, Model, lod3,
                                            Low::Renderer::Mesh);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {
          Model l_Handle = p_Handle.get_id();
          l_Handle.set_lod3(*(Low::Renderer::Mesh *)p_Data);
        };
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle,
                                void *p_Data) {
          Model l_Handle = p_Handle.get_id();
          *((Low::Renderer::Mesh *)p_Data) = l_Handle.get_lod3();
        };
        l_TypeInfo.properties[l_PropertyInfo.name] = l_PropertyInfo;
        // End property: lod3
      }
      {
        // Property: unique_id
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
        l_PropertyInfo.name = N(unique_id);
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset = offsetof(Model::Data, unique_id);
        l_PropertyInfo.size = sizeof(Model::Data::unique_id);
        l_PropertyInfo.type = Low::Util::RTTI::PropertyType::UINT64;
        l_PropertyInfo.handleType = 0;
        l_PropertyInfo.get_return =
            [](Low::Util::Handle p_Handle) -> void const * {
          Model l_Handle = p_Handle.get_id();
          l_Handle.get_unique_id();
          return (void *)&ACCESSOR_TYPE_SOA(
              p_Handle, Model, unique_id, Low::Util::UniqueId);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {};
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle,
                                void *p_Data) {
          Model l_Handle = p_Handle.get_id();
          *((Low::Util::UniqueId *)p_Data) = l_Handle.get_unique_id();
        };
        l_TypeInfo.properties[l_PropertyInfo.name] = l_PropertyInfo;
        // End property: unique_id
      }
      {
        // Property: name
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
        l_PropertyInfo.name = N(name);
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset = offsetof(Model::Data, name);
        l_PropertyInfo.size = sizeof(Model::Data::name);
        l_PropertyInfo.type = Low::Util::RTTI::PropertyType::NAME;
        l_PropertyInfo.handleType = 0;
        l_PropertyInfo.get_return =
            [](Low::Util::Handle p_Handle) -> void const * {
          Model l_Handle = p_Handle.get_id();
          l_Handle.get_name();
          return (void *)&ACCESSOR_TYPE_SOA(p_Handle, Model, name,
                                            Low::Util::Name);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {
          Model l_Handle = p_Handle.get_id();
          l_Handle.set_name(*(Low::Util::Name *)p_Data);
        };
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle,
                                void *p_Data) {
          Model l_Handle = p_Handle.get_id();
          *((Low::Util::Name *)p_Data) = l_Handle.get_name();
        };
        l_TypeInfo.properties[l_PropertyInfo.name] = l_PropertyInfo;
        // End property: name
      }
      ms_TypeId = Low::Util::Handle::register_type_info(IDENTIFIER,
                                                        l_TypeInfo);
      // LOW_CODEGEN:BEGIN:CUSTOM:POSTINITIALIZE

      // LOW_CODEGEN::END::CUSTOM:POSTINITIALIZE
    }

    void Model::cleanup()
    {
      Low::Util::List<Model> l_Instances = ms_LivingInstances;
      for (uint32_t i = 0u; i < l_Instances.size(); ++i) {
        l_Instances[i].destroy();
      }
      for (auto it = ms_Pages.begin(); it != ms_Pages.end();) {
        Low::Util::Instances::Page *i_Page = *it;
        free(i_Page->buffer);
        free(i_Page->slots);
        delete i_Page;
        it = ms_Pages.erase(it);
      }

      ms_Capacity = 0;
    }

    Low::Util::Handle Model::_find_by_index(uint32_t p_Index)
    {
      return find_by_index(p_Index).get_id();
    }

    Model Model::find_by_index(uint32_t p_Index)
    {
      LOW_ASSERT(p_Index < get_capacity(), "Index out of bounds");

      Model l_Handle;
      l_Handle.m_Data.m_Index = p_Index;
      l_Handle.m_Data.m_Type = Model::ms_TypeId;

      u32 l_PageIndex = 0;
      u32 l_SlotIndex = 0;
      if (!get_page_for_index(p_Index, l_PageIndex, l_SlotIndex)) {
        l_Handle.m_Data.m_Generation = 0;
      }
      Low::Util::Instances::Page *l_Page = ms_Pages[l_PageIndex];
      l_Handle.m_Data.m_Generation =
          l_Page->slots[l_SlotIndex].m_Generation;

      return l_Handle;
    }

    Model Model::create_handle_by_index(u32 p_Index)
    {
      if (p_Index < get_capacity()) {
        return find_by_index(p_Index);
      }

      Model l_Handle;
      l_Handle.m_Data.m_Index = p_Index;
      l_Handle.m_Data.m_Generation = 0;
      l_Handle.m_Data.m_Type = Model::ms_TypeId;

      return l_Handle;
    }

    bool Model::is_alive() const
    {
      if (m_Data.m_Type != Model::ms_TypeId) {
        return false;
      }
      u32 l_PageIndex = 0;
      u32 l_SlotIndex = 0;
      if (!get_page_for_index(get_index(), l_PageIndex,
                              l_SlotIndex)) {
        return false;
      }
      Low::Util::Instances::Page *l_Page = ms_Pages[l_PageIndex];
      return m_Data.m_Type == Model::ms_TypeId &&
             l_Page->slots[l_SlotIndex].m_Occupied &&
             l_Page->slots[l_SlotIndex].m_Generation ==
                 m_Data.m_Generation;
    }

    uint32_t Model::get_capacity()
    {
      return ms_Capacity;
    }

    Low::Util::Handle Model::_find_by_name(Low::Util::Name p_Name)
    {
      return find_by_name(p_Name).get_id();
    }

    Model Model::find_by_name(Low::Util::Name p_Name)
    {

      // LOW_CODEGEN:BEGIN:CUSTOM:FIND_BY_NAME

      // LOW_CODEGEN::END::CUSTOM:FIND_BY_NAME

      for (auto it = ms_LivingInstances.begin();
           it != ms_LivingInstances.end(); ++it) {
        if (it->get_name() == p_Name) {
          return *it;
        }
      }
      return Low::Util::Handle::DEAD;
    }

    Model Model::duplicate(Low::Util::Name p_Name) const
    {
      _LOW_ASSERT(is_alive());

      Model l_Handle = make(p_Name);
      if (get_resource().is_alive()) {
        l_Handle.set_resource(get_resource());
      }
      if (get_lod0().is_alive()) {
        l_Handle.set_lod0(get_lod0());
      }
      if (get_lod1().is_alive()) {
        l_Handle.set_lod1(get_lod1());
      }
      if (get_lod2().is_alive()) {
        l_Handle.set_lod2(get_lod2());
      }
      if (get_lod3().is_alive()) {
        l_Handle.set_lod3(get_lod3());
      }

      // LOW_CODEGEN:BEGIN:CUSTOM:DUPLICATE

      // LOW_CODEGEN::END::CUSTOM:DUPLICATE

      return l_Handle;
    }

    Model Model::duplicate(Model p_Handle, Low::Util::Name p_Name)
    {
      return p_Handle.duplicate(p_Name);
    }

    Low::Util::Handle Model::_duplicate(Low::Util::Handle p_Handle,
                                        Low::Util::Name p_Name)
    {
      Model l_Model = p_Handle.get_id();
      return l_Model.duplicate(p_Name);
    }

    void Model::serialize(Low::Util::Serial::Node &p_Node) const
    {
      _LOW_ASSERT(is_alive());

      if (get_resource().is_alive()) {
        get_resource().serialize(p_Node["resource"]);
      }
      p_Node["_unique_id"] = Low::Util::U64Id{get_unique_id()};
      p_Node["name"] = get_name().c_str();

      // LOW_CODEGEN:BEGIN:CUSTOM:SERIALIZER

      if (get_lod0().is_alive()) {
        p_Node["lod0"] =
            Util::hash_to_string(get_lod0().get_unique_id()).c_str();
      }
      if (get_lod1().is_alive()) {
        p_Node["lod1"] =
            Util::hash_to_string(get_lod1().get_unique_id()).c_str();
      }
      if (get_lod2().is_alive()) {
        p_Node["lod2"] =
            Util::hash_to_string(get_lod2().get_unique_id()).c_str();
      }
      if (get_lod3().is_alive()) {
        p_Node["lod3"] =
            Util::hash_to_string(get_lod3().get_unique_id()).c_str();
      }
      // LOW_CODEGEN::END::CUSTOM:SERIALIZER
    }

    void Model::serialize(Low::Util::Handle p_Handle,
                          Low::Util::Serial::Node &p_Node)
    {
      Model l_Model = p_Handle.get_id();
      l_Model.serialize(p_Node);
    }

    Low::Util::Handle
    Model::deserialize(Low::Util::Serial::Node &p_Node,
                       Low::Util::Handle p_Creator)
    {
      Low::Util::UniqueId l_HandleUniqueId = 0ull;
      if (p_Node["unique_id"]) {
        l_HandleUniqueId = p_Node["unique_id"].as<uint64_t>();
      } else if (p_Node["_unique_id"]) {
        l_HandleUniqueId = Low::Util::string_to_hash(
            p_Node["_unique_id"].as<Low::Util::String>());
      }

      Model l_Handle = Model::make(N(Model), l_HandleUniqueId);

      if (p_Node["resource"]) {
        l_Handle.set_resource(
            Low::Renderer::ModelResource::deserialize(
                p_Node["resource"], l_Handle.get_id())
                .get_id());
      }
      if (p_Node["unique_id"]) {
        l_Handle.set_unique_id(
            p_Node["unique_id"].as<Low::Util::UniqueId>());
      }
      if (p_Node["name"]) {
        l_Handle.set_name(p_Node["name"].as<Low::Util::Name>());
      }

      // LOW_CODEGEN:BEGIN:CUSTOM:DESERIALIZER

      if (p_Node["lod0"]) {
        l_Handle.set_lod0(ResourceManager::find_asset<Mesh>(
            p_Node["lod0"].as<Util::U64Id>().val));
      }
      if (p_Node["lod1"]) {
        l_Handle.set_lod1(ResourceManager::find_asset<Mesh>(
            p_Node["lod1"].as<Util::U64Id>().val));
      }
      if (p_Node["lod2"]) {
        l_Handle.set_lod3(ResourceManager::find_asset<Mesh>(
            p_Node["lod2"].as<Util::U64Id>().val));
      }
      if (p_Node["lod3"]) {
        l_Handle.set_lod3(ResourceManager::find_asset<Mesh>(
            p_Node["lod3"].as<Util::U64Id>().val));
      }

      ResourceManager::register_asset(l_Handle.get_unique_id(),
                                      l_Handle);
      // LOW_CODEGEN::END::CUSTOM:DESERIALIZER

      return l_Handle;
    }

    void
    Model::broadcast_observable(Low::Util::Name p_Observable) const
    {
      Low::Util::ObserverKey l_Key;
      l_Key.handleId = get_id();
      l_Key.observableName = p_Observable.m_Index;

      Low::Util::notify(l_Key);
    }

    u64 Model::observe(
        Low::Util::Name p_Observable,
        Low::Util::Function<void(Low::Util::Handle, Low::Util::Name)>
            p_Observer) const
    {
      Low::Util::ObserverKey l_Key;
      l_Key.handleId = get_id();
      l_Key.observableName = p_Observable.m_Index;

      return Low::Util::observe(l_Key, p_Observer);
    }

    u64 Model::observe(Low::Util::Name p_Observable,
                       Low::Util::Handle p_Observer) const
    {
      Low::Util::ObserverKey l_Key;
      l_Key.handleId = get_id();
      l_Key.observableName = p_Observable.m_Index;

      return Low::Util::observe(l_Key, p_Observer);
    }

    void Model::notify(Low::Util::Handle p_Observed,
                       Low::Util::Name p_Observable)
    {
      // LOW_CODEGEN:BEGIN:CUSTOM:NOTIFY

      // LOW_CODEGEN::END::CUSTOM:NOTIFY
    }

    void Model::_notify(Low::Util::Handle p_Observer,
                        Low::Util::Handle p_Observed,
                        Low::Util::Name p_Observable)
    {
      Model l_Model = p_Observer.get_id();
      l_Model.notify(p_Observed, p_Observable);
    }

    Low::Renderer::ModelResource Model::get_resource() const
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_resource

      // LOW_CODEGEN::END::CUSTOM:GETTER_resource

      return TYPE_SOA(Model, resource, Low::Renderer::ModelResource);
    }
    void Model::set_resource(Low::Renderer::ModelResource p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_resource

      // LOW_CODEGEN::END::CUSTOM:PRESETTER_resource

      // Set new value
      TYPE_SOA(Model, resource, Low::Renderer::ModelResource) =
          p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_resource

      // LOW_CODEGEN::END::CUSTOM:SETTER_resource

      broadcast_observable(N(resource));
    }

    Low::Renderer::Mesh Model::get_lod0() const
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_lod0

      // LOW_CODEGEN::END::CUSTOM:GETTER_lod0

      return TYPE_SOA(Model, lod0, Low::Renderer::Mesh);
    }
    void Model::set_lod0(Low::Renderer::Mesh p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_lod0

      // LOW_CODEGEN::END::CUSTOM:PRESETTER_lod0

      // Set new value
      TYPE_SOA(Model, lod0, Low::Renderer::Mesh) = p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_lod0

      // LOW_CODEGEN::END::CUSTOM:SETTER_lod0

      broadcast_observable(N(lod0));
    }

    Low::Renderer::Mesh Model::get_lod1() const
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_lod1

      // LOW_CODEGEN::END::CUSTOM:GETTER_lod1

      return TYPE_SOA(Model, lod1, Low::Renderer::Mesh);
    }
    void Model::set_lod1(Low::Renderer::Mesh p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_lod1

      // LOW_CODEGEN::END::CUSTOM:PRESETTER_lod1

      // Set new value
      TYPE_SOA(Model, lod1, Low::Renderer::Mesh) = p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_lod1

      // LOW_CODEGEN::END::CUSTOM:SETTER_lod1

      broadcast_observable(N(lod1));
    }

    Low::Renderer::Mesh Model::get_lod2() const
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_lod2

      // LOW_CODEGEN::END::CUSTOM:GETTER_lod2

      return TYPE_SOA(Model, lod2, Low::Renderer::Mesh);
    }
    void Model::set_lod2(Low::Renderer::Mesh p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_lod2

      // LOW_CODEGEN::END::CUSTOM:PRESETTER_lod2

      // Set new value
      TYPE_SOA(Model, lod2, Low::Renderer::Mesh) = p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_lod2

      // LOW_CODEGEN::END::CUSTOM:SETTER_lod2

      broadcast_observable(N(lod2));
    }

    Low::Renderer::Mesh Model::get_lod3() const
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_lod3

      // LOW_CODEGEN::END::CUSTOM:GETTER_lod3

      return TYPE_SOA(Model, lod3, Low::Renderer::Mesh);
    }
    void Model::set_lod3(Low::Renderer::Mesh p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_lod3

      // LOW_CODEGEN::END::CUSTOM:PRESETTER_lod3

      // Set new value
      TYPE_SOA(Model, lod3, Low::Renderer::Mesh) = p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_lod3

      // LOW_CODEGEN::END::CUSTOM:SETTER_lod3

      broadcast_observable(N(lod3));
    }

    Low::Util::UniqueId Model::get_unique_id() const
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_unique_id

      // LOW_CODEGEN::END::CUSTOM:GETTER_unique_id

      return TYPE_SOA(Model, unique_id, Low::Util::UniqueId);
    }
    void Model::set_unique_id(Low::Util::UniqueId p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_unique_id

      // LOW_CODEGEN::END::CUSTOM:PRESETTER_unique_id

      // Set new value
      TYPE_SOA(Model, unique_id, Low::Util::UniqueId) = p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_unique_id

      // LOW_CODEGEN::END::CUSTOM:SETTER_unique_id

      broadcast_observable(N(unique_id));
    }

    Low::Util::Name Model::get_name() const
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_name

      // LOW_CODEGEN::END::CUSTOM:GETTER_name

      return TYPE_SOA(Model, name, Low::Util::Name);
    }
    void Model::set_name(Low::Util::Name p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_name

      // LOW_CODEGEN::END::CUSTOM:PRESETTER_name

      // Set new value
      TYPE_SOA(Model, name, Low::Util::Name) = p_Value;

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_name

      // LOW_CODEGEN::END::CUSTOM:SETTER_name

      broadcast_observable(N(name));
    }

    uint32_t Model::create_instance(u32 &p_PageIndex,
                                    u32 &p_SlotIndex)
    {
      u32 l_Index = 0;
      u32 l_PageIndex = 0;
      u32 l_SlotIndex = 0;
      bool l_FoundIndex = false;

      for (; !l_FoundIndex && l_PageIndex < ms_Pages.size();
           ++l_PageIndex) {
        for (l_SlotIndex = 0;
             l_SlotIndex < ms_Pages[l_PageIndex]->size;
             ++l_SlotIndex) {
          if (!ms_Pages[l_PageIndex]->slots[l_SlotIndex].m_Occupied) {
            l_FoundIndex = true;
            break;
          }
          l_Index++;
        }
        if (l_FoundIndex) {
          break;
        }
      }
      if (!l_FoundIndex) {
        l_SlotIndex = 0;
        l_PageIndex = create_page();
      }
      ms_Pages[l_PageIndex]->slots[l_SlotIndex].m_Occupied = true;
      p_PageIndex = l_PageIndex;
      p_SlotIndex = l_SlotIndex;
      return l_Index;
    }

    u32 Model::create_page()
    {
      const u32 l_Capacity = get_capacity();
      LOW_ASSERT((l_Capacity + ms_PageSize) < LOW_UINT32_MAX,
                 "Could not increase capacity for Model.");

      Low::Util::Instances::Page *l_Page =
          new Low::Util::Instances::Page;
      Low::Util::Instances::initialize_page(
          l_Page, Model::Data::get_size(), ms_PageSize);
      ms_Pages.push_back(l_Page);

      ms_Capacity = l_Capacity + l_Page->size;
      return ms_Pages.size() - 1;
    }

    bool Model::get_page_for_index(const u32 p_Index,
                                   u32 &p_PageIndex, u32 &p_SlotIndex)
    {
      if (p_Index >= get_capacity()) {
        p_PageIndex = LOW_UINT32_MAX;
        p_SlotIndex = LOW_UINT32_MAX;
        return false;
      }
      p_PageIndex = p_Index / ms_PageSize;
      if (p_PageIndex > (ms_Pages.size() - 1)) {
        return false;
      }
      p_SlotIndex = p_Index - (ms_PageSize * p_PageIndex);
      return true;
    }

    // LOW_CODEGEN:BEGIN:CUSTOM:NAMESPACE_AFTER_TYPE_CODE

    // LOW_CODEGEN::END::CUSTOM:NAMESPACE_AFTER_TYPE_CODE

  } // namespace Renderer
} // namespace Low
