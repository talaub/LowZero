...
namespace Low {
  namespace Renderer {
    ...
    void RenderObject::initialize()
    {
      ...
      {
        // Property: render_view_handle
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
        l_PropertyInfo.name = N(render_view_handle);
        l_PropertyInfo.editorProperty = false;
        l_PropertyInfo.dataOffset =
            offsetof(RenderObjectData, render_view_handle);
        l_PropertyInfo.type = Low::Util::RTTI::PropertyType::UINT64;
        l_PropertyInfo.handleType = 0;
        l_PropertyInfo.get_return =
            [](Low::Util::Handle p_Handle) -> void const * {
          RenderObject l_Handle = p_Handle.get_id();
          l_Handle.get_render_view_handle();
          return (void *)&ACCESSOR_TYPE_SOA(
              p_Handle, RenderObject, render_view_handle, uint64_t);
        };
        l_PropertyInfo.set = [](Low::Util::Handle p_Handle,
                                const void *p_Data) -> void {};
        l_PropertyInfo.get = [](Low::Util::Handle p_Handle,
                                void *p_Data) {
          RenderObject l_Handle = p_Handle.get_id();
          *((uint64_t *)p_Data) = l_Handle.get_render_view_handle();
        };
        l_TypeInfo.properties[l_PropertyInfo.name] = l_PropertyInfo;
        // End property: render_view_handle
      }
      {
        // Property: dirty
        Low::Util::RTTI::PropertyInfo l_PropertyInfo;
      ...
      }
      {
        ...
        l_FunctionInfo.handleType = RenderObject::TYPE_ID;
        {
          ...
          l_ParameterInfo.handleType = RenderView::TYPE_ID;
    ...
    RenderObject RenderObject::duplicate(Low::Util::Name p_Name) const
    {
      ...
      l_Handle.set_render_view_handle(get_render_view_handle());
    ...
    uint64_t RenderObject::get_render_view_handle() const
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:GETTER_render_view_handle
      // LOW_CODEGEN::END::CUSTOM:GETTER_render_view_handle

      READ_LOCK(l_ReadLock);
      return TYPE_SOA(RenderObject, render_view_handle, uint64_t);
    }
    void RenderObject::set_render_view_handle(uint64_t p_Value)
    {
      _LOW_ASSERT(is_alive());

      // LOW_CODEGEN:BEGIN:CUSTOM:PRESETTER_render_view_handle
      // LOW_CODEGEN::END::CUSTOM:PRESETTER_render_view_handle

      // Set new value
      WRITE_LOCK(l_WriteLock);
      TYPE_SOA(RenderObject, render_view_handle, uint64_t) = p_Value;
      LOCK_UNLOCK(l_WriteLock);

      // LOW_CODEGEN:BEGIN:CUSTOM:SETTER_render_view_handle
      // LOW_CODEGEN::END::CUSTOM:SETTER_render_view_handle
    }
    ...
    RenderObject RenderObject::make(RenderView p_RenderView)
    {
      // LOW_CODEGEN:BEGIN:CUSTOM:FUNCTION_make
      _LOW_ASSERT(p_RenderView.is_alive());

      RenderObject l_Handle = make(N(RenderObject));
      l_Handle.set_render_view_handle(p_RenderView.get_id());
