...
void main() {
    ...
    vec4 result = texture(g_Texture2Ds[params.inputTextureIndex], texCoords) * weights[0];

    // Horizontal blur
    for (int i = 1; i < 3; ++i) {
        result += texture(g_Texture2Ds[params.inputTextureIndex], texCoords + vec2(params.texelSize.x * i, 0.0)) * weights[i];
        result += texture(g_Texture2Ds[params.inputTextureIndex], texCoords - vec2(params.texelSize.x * i, 0.0)) * weights[i];
    }
