...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float frame_height = ImMax(GetFrameHeight(), 27.0f);
    const float frame_rounding = ImMax(style.FrameRounding, 4.0f);
    const float arrow_size = (flags & ImGuiComboFlags_NoArrowButton) ? 0.0f : frame_height;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, frame_height));
    ...
    // Render shape
    const ImU32 frame_col = GetColorU32((popup_open || hovered) ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    RenderNavCursor(bb, id);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, frame_col, frame_rounding, ImDrawFlags_RoundCornersAll);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        const ImRect arrow_bb(ImVec2(value_x2, bb.Min.y), bb.Max);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        const ImVec2 arrow_center = arrow_bb.GetCenter();
        const float divider_padding = ImMax(2.0f, style.FramePadding.y * 0.75f);
        const float border_size = ImMax(1.0f, style.FrameBorderSize);
        const ImU32 border_col = GetColorU32(ImGuiCol_Border);
        if (!(flags & ImGuiComboFlags_NoPreview) && value_x2 > bb.Min.x)
            window->DrawList->AddLine(ImVec2(value_x2, bb.Min.y + divider_padding), ImVec2(value_x2, bb.Max.y - divider_padding), border_col, border_size);
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
        {
            const char* chevron = popup_open ? ICON_LC_CHEVRON_UP : ICON_LC_CHEVRON_DOWN;
            const ImVec2 chevron_size = CalcTextSize(chevron);
            window->DrawList->AddText(arrow_center - chevron_size * 0.5f, text_col, chevron);
        }
        window->DrawList->AddRect(bb.Min, bb.Max, border_col, frame_rounding, 0, border_size);
    }
    else
    {
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), frame_rounding, 0, ImMax(1.0f, style.FrameBorderSize));
    }
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        const float preview_y = bb.Min.y + ImMax(style.FramePadding.y, (bb.GetHeight() - label_size.y) * 0.5f);
        RenderTextClipped(ImVec2(bb.Min.x + style.FramePadding.x, preview_y), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    }
    if (label_size.x > 0)
