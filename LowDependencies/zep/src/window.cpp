...
void ZepWindow::Notify(std::shared_ptr<ZepMessage> payload)
{
    ...
    else if (payload->messageId == Msg::MouseDown)
    {
        if (payload->button == ZepMouseButton::Left && m_textRegion->rect.Contains(m_mousePos) && m_mouseIterator.Valid())
        {
            ...
            m_mouseSelectionAnchor = m_mouseIterator;
            m_pBuffer->ClearSelection();
            if (m_pBuffer->GetMode())
            {
                m_pBuffer->GetMode()->SwitchMode(m_pBuffer->GetMode()->DefaultMode());
            }
    ...
    else if (payload->messageId == Msg::MouseUp)
    {
        if (payload->button == ZepMouseButton::Left)
        {
            ...
            m_mouseSelectionAnchor.Invalidate();
        }
    }
...
void ZepWindow::UpdateMouseSelection()
{
    if (!m_mouseSelecting || !m_mouseSelectionAnchor.Valid() ||
        !m_mouseIterator.Valid() || !m_pBuffer || !m_pBuffer->GetMode())
    {
        return;
    }

    if (m_mouseSelectionAnchor == m_mouseIterator)
    {
        m_pBuffer->ClearSelection();
        return;
    }
    ...
    SetBufferCursor(m_mouseIterator);
    m_pBuffer->GetMode()->SetVisualSelection(m_mouseSelectionAnchor,
                                             m_mouseIterator);
}
