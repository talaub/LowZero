...
//   following license: you are granted a perpetual, irrevocable
//   license to copy, modify, publish, and distribute this file as you
//   see fit.
//
// CREDITS
...
#define DECLARE_KEY_TESTER(Key)                                      \
  ...
  {                                                                  \
    template <typename T>                                            \
    static int                                                       \
    Get(typename std::enable_if<has_nested_##Key<ImGuiKey_>::value,  \
                                T>::type *)                          \
    {                                                                \
...
static const int c_NodeStartChannel =
    c_LinkStartChannel + c_LinkChannelCount;
...
static const int c_UserChannel_HintsBackground =
    c_UserLayerChannelStart + 3;
...
static const auto c_AllRoundCornersFlags =
...
static void ImDrawList_ChannelsGrow(ImDrawList *draw_list,
                                    int channels_count)
{
  ImDrawListSplitter_Grow(draw_list, &draw_list->_Splitter,
                          channels_count);
}
...
static void ImDrawList_SwapChannels(ImDrawList *drawList, int left,
                                    int right)
{
...
// vtxBuffer, const ImVector<ImDrawIdx>& idxBuffer, const
// ImVector<ImDrawCmd>& cmdBuffer, const ImVec2& preOffset, const
...
//         }
//
//         for (auto vtx = vtxBuffer.Data + minIndex, vtxEnd =
//         vtxBuffer.Data + maxIndex + 1; vtx < vtxEnd; ++vtx)
//         {
//             vtx->pos.x = (vtx->pos.x + preOffset.x) * scale.x +
//             postOffset.x; vtx->pos.y = (vtx->pos.y + preOffset.y) *
//             scale.y + postOffset.y;
//         }
//
...
// static void ImDrawList_TransformChannels(ImDrawList* drawList, int
// begin, int end, const ImVec2& preOffset, const ImVec2& scale, const
// ImVec2& postOffset)
//{
...
//     {
//         ImDrawList_TransformChannel_Inner(vtxBuffer,
//         drawList->IdxBuffer, drawList->CmdBuffer, preOffset, scale,
//         postOffset);
//         ++begin;
//     }
//
//     for (int channelIndex = begin; channelIndex < end;
//     ++channelIndex)
//     {
//         auto& channel = drawList->_Channels[channelIndex];
//         ImDrawList_TransformChannel_Inner(vtxBuffer,
//         channel.IdxBuffer, channel.CmdBuffer, preOffset, scale,
//         postOffset);
//     }
//
//     if (lastCurrentChannel != 0)
...
// static void ImDrawList_ClampClipRects_Inner(ImVector<ImDrawCmd>&
// cmdBuffer, const ImVec4& clipRect, const ImVec2& offset)
//{
//     for (auto& cmd : cmdBuffer)
//     {
//         cmd.ClipRect.x = ImMax(cmd.ClipRect.x + offset.x,
//         clipRect.x); cmd.ClipRect.y = ImMax(cmd.ClipRect.y +
//         offset.y, clipRect.y); cmd.ClipRect.z =
//         std::min(cmd.ClipRect.z + offset.x, clipRect.z);
//         cmd.ClipRect.w = std::min(cmd.ClipRect.w + offset.y,
//         clipRect.w);
//     }
// }

// static void ImDrawList_TranslateAndClampClipRects(ImDrawList*
// drawList, int begin, int end, const ImVec2& offset)
//{
//     int lastCurrentChannel = drawList->_ChannelsCurrent;
//     if (lastCurrentChannel != 0)
//         drawList->ChannelsSetCurrent(0);
...
//     if (begin == 0 && begin != end)
//     {
//         ImDrawList_ClampClipRects_Inner(drawList->CmdBuffer,
//         clipRect, offset);
//         ++begin;
//     }
//
...
//     {
//         auto& channel = drawList->_Channels[channelIndex];
//         ImDrawList_ClampClipRects_Inner(channel.CmdBuffer,
//         clipRect, offset);
//     }
//
//     if (lastCurrentChannel != 0)
...
static void
ImDrawList_PathBezierOffset(ImDrawList *drawList, float offset,
                            ...
                            const ImVec2 &p2, const ImVec2 &p3)
{
  ...
  auto acceptPoint = [drawList,
    ...
    drawList->PathLineTo(
        r.Point +
        ImNormalized(ImVec2(-r.Tangent.y, r.Tangent.x)) * offset);
  };
...
/*
static void ImDrawList_PolyFillScanFlood(ImDrawList *draw,
std::vector<ImVec2>* poly, ImColor color, int gap = 1, float
strokeWidth = 1.0f)
{
    ...
    // Bounds check
    if ((max.x < 0) || (min.x > io.DisplaySize.x) || (max.y < 0) ||
(min.y > io.DisplaySize.y)) return;

    // Vertically clip
    if (min.y < 0) min.y = 0;
    if (max.y > io.DisplaySize.y) max.y = io.DisplaySize.y;

    // so we know we start on the outside of the object we step out
by 1. min.x -= 1; max.x += 1;

    // Initialise our starting conditions
    y = min.y;

    // Go through each scan line iteratively, jumping by 'gap' pixels
each time while (y < max.y)
    {
        scanHits.clear();

        {
            ...
            for (size_t i = 0; i < polysize - 1; i++)
            {
                ...
                // if we encounter our hull/poly start point, then
we've now created the
                // closed
                // hull, jump the next segment and reset the
first-point if ((!jump) && (fp.x == pb.x) && (fp.y == pb.y))
                {
                    if (i < polysize - 2)
                    {
                        fp = poly->at(i + 2);
                ...
                if ((pa.y > pb.y && y < pa.y && y > pb.y) || (pa.y <
pb.y && y > pa.y && y < pb.y))
                {
                    ImVec2 intersect;
                    ...
                    {
                        intersect.x = (pb.x - pa.x) / (pb.y - pa.y) *
(y - pa.y) + pa.x;
                    }
            ...
            // Sort the scan hits by X, so we have a proper
left->right ordering sort(scanHits.begin(), scanHits.end(), [](ImVec2
const &a, ImVec2 const &b) { return a.x < b.x; });

            // generate the line segments.
            {
                int i = 0;
                int l = scanHits.size() - 1; // we need pairs of
points, this prevents segfault. for (i = 0; i < l; i += 2)
                {
...
static void ImDrawList_AddBezierWithArrows(
    ImDrawList *drawList, const ImCubicBezierPoints &curve,
    float thickness, float startArrowSize, float startArrowWidth,
    float endArrowSize, float endArrowWidth, bool fill, ImU32 color,
    float strokeThickness, const ImVec2 *startDirHint = nullptr,
    const ImVec2 *endDirHint = nullptr)
{
  ...
  if (fill) {
    ...
    ImDrawList_PathBezierOffset(drawList, half_thickness, curve.P0,
    ...
    ImDrawList_PathBezierOffset(drawList, half_thickness, curve.P3,
                                curve.P2, curve.P1, curve.P0);
...
{
  if (flags & Hovered) {
    drawList->ChannelsSetCurrent(m_Node->m_Channel +
                                 c_NodePinChannel);

    drawList->AddRectFilled(m_Bounds.Min, m_Bounds.Max, m_Color,
                            m_Rounding, m_Corners);

    if (m_BorderWidth > 0.0f) {
      FringeScaleScope fringe(1.0f);
      drawList->AddRect(m_Bounds.Min, m_Bounds.Max, m_BorderColor,
                        m_Rounding, m_Corners, m_BorderWidth);
    }
...
void ed::Node::DrawBorder(ImDrawList *drawList, ImU32 color,
                          float thickness, float offset)
{
  if (thickness > 0.0f) {
    const ImVec2 extraOffset = ImVec2(offset, offset);

    drawList->AddRect(m_Bounds.Min - extraOffset,
                      m_Bounds.Max + extraOffset, color,
                      ImMax(0.0f, m_Rounding + offset),
                      c_AllRoundCornersFlags, thickness);
  }
}

void ed::Node::GetGroupedNodes(std::vector<Node *> &result,
                               bool append)
{
  if (!append)
    result.resize(0);
...
ImRect ed::Node::GetRegionBounds(NodeRegion region) const
{
  if (m_Type == NodeType::Node) {
    ...
    const float activeAreaMinimumSize = ImMax(
        ImMax(Editor->GetView().InvScale * c_GroupSelectThickness,
              m_GroupBorderWidth),
    ...
    if (region == NodeRegion::Top) {
      ...
      bounds.Max.x -= activeAreaMinimumSize;
      bounds.Min.y += activeAreaMinimumSize;
      bounds.Max.y = ImMax(bounds.Min.y + activeAreaMinimumSize,
                           m_GroupBounds.Min.y);
      return bounds;
    } else if (region == NodeRegion::Center) {
      bounds.Max.x -= activeAreaMinimumSize;
      bounds.Min.y = ImMax(bounds.Min.y + activeAreaMinimumSize,
                           m_GroupBounds.Min.y);
      bounds.Min.x += activeAreaMinimumSize;
      bounds.Max.y -= activeAreaMinimumSize;
      return bounds;
                    ...
                    float extraThickness) const
{
  ...
  ImDrawList_AddBezierWithArrows(
      ...
      m_StartPin && m_StartPin->m_SnapLinkToDir ? &m_StartPin->m_Dir
                                                : nullptr,
      m_EndPin && m_EndPin->m_SnapLinkToDir ? &m_EndPin->m_Dir
                                            : nullptr);
}
...
{
  auto easeLinkStrength = [](const ImVec2 &a, const ImVec2 &b,
                             float strength) {
    ...
    if (halfDistance < strength)
      strength =
          strength * ImSin(IM_PI * 0.5f * halfDistance / strength);
...
bool ed::Link::TestHit(const ImVec2 &point,
                       float extraThickness) const
{
  if (!m_IsLive)
    return false;
...
bool ed::Link::TestHit(const ImRect &rect, bool allowIntersect) const
{
  ...
  if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2,
                                 bezier.P3, p0, p1)
          .Count > 0)
    return true;
  if (ImCubicBezierLineIntersect(bezier.P0, bezier.P1, bezier.P2,
...
ImRect ed::Link::GetBounds() const
{
  if (m_IsLive) {
    const auto curve = GetCurve();
    auto bounds = ImCubicBezierBoundingRect(curve.P0, curve.P1,
                                            curve.P2, curve.P3);
...
ed::EditorContext::EditorContext(const ax::NodeEditor::Config *config)
    : m_Config(config), m_EditorActiveId(0), m_IsFirstFrame(true),
      m_IsFocused(false), m_IsHovered(false),
      m_IsHoveredWithoutOverlapp(false), m_ShortcutsEnabled(true),
      m_Style(), m_Nodes(), m_Pins(), m_Links(), m_SelectionId(1),
      m_LastActiveLink(nullptr), m_Canvas(), m_IsCanvasVisible(false),
      m_NodeBuilder(this), m_HintBuilder(this),
      ...
      m_FlowAnimationController(this), m_HoveredNode(0),
      m_HoveredPin(0), m_HoveredLink(0), m_DoubleClickedNode(0),
      m_DoubleClickedPin(0), m_DoubleClickedLink(0),
...
void ed::EditorContext::Begin(const char *id, const ImVec2 &size)
{
  ...
  if (!m_IsInitialized) {
    // Cycle canvas so it has a change to setup its size before
    // settings are loaded
  ...
  if (!ImRect_IsEmpty(previousVisibleRect) &&
      previousSize != canvasSize) {
    m_NavigateAction.FinishNavigation();

    auto centerX =
        (previousVisibleRect.Max.x + previousVisibleRect.Min.x) *
        0.5f;
    auto centerY =
        (previousVisibleRect.Max.y + previousVisibleRect.Min.y) *
        0.5f;
    ...
    m_NavigateAction.NavigateTo(
        previousVisibleRect, Detail::NavigateAction::ZoomMode::Exact,
        0.0f);
  }
  ...
  //    clipMin.x, clipMin.y, clipMax.x - clipMin.x, clipMax.y -
  //    clipMin.y, clipMax.x, clipMax.y);
...
void ed::EditorContext::End()
{
  ...
  auto control = BuildControl(
      m_CurrentAction &&
      m_CurrentAction
          ->IsDragging()); // NavigateAction.IsMovingOverEdge()
  ...
  // DoubleClickedNode); if (DoubleClickedPin)  LOG_TRACE(0, "DOUBLE
  // CLICK PIN:  %d", DoubleClickedPin); if (DoubleClickedLink)
  // LOG_TRACE(0, "DOUBLE CLICK LINK: %d", DoubleClickedLink); if
  // (BackgroundDoubleClicked) LOG_TRACE(0, "DOUBLE CLICK BACKGROUND",
  // DoubleClickedLink);
  ...
  if (nullptr == m_CurrentAction) {
    ...
    auto accept = [&possibleAction, &control](EditorAction &action) {
      ...
      if (result == EditorAction::True)
        return true;
      else if (/*!possibleAction &&*/ result ==
               EditorAction::Possible)
  ...
  if (control.ActiveNode) {
    if (!IsGroup(control.ActiveNode)) {
      ...
      auto activeNodeIt = std::find(m_Nodes.begin(), m_Nodes.end(),
                                    control.ActiveNode);
      std::rotate(activeNodeIt, activeNodeIt + 1, m_Nodes.end());
    } else if (!isDragging && m_CurrentAction &&
  ...
  if (sortGroups ||
    ...
    auto groupsItEnd = std::stable_partition(m_Nodes.begin(),
                                             m_Nodes.end(), IsGroup);

    // Sort groups by area
    std::sort(m_Nodes.begin(), groupsItEnd,
...
#if 1 // #FIXME
  {
    ...
    ImVec2 offset =
        m_Canvas.ViewOrigin() * (1.0f / m_Canvas.ViewScale());
    ImU32 GRID_COLOR =
        GetColor(StyleColor_Grid,
                 ImClamp(m_Canvas.ViewScale() * m_Canvas.ViewScale(),
                         0.0f, 1.0f));
    ...
    for (float x = fmodf(offset.x, GRID_SX); x < VIEW_SIZE.x;
         x += GRID_SX)
      m_DrawList->AddLine(ImVec2(x, 0.0f) + VIEW_POS,
                          ImVec2(x, VIEW_SIZE.y) + VIEW_POS,
                          GRID_COLOR);
    for (float y = fmodf(offset.y, GRID_SY); y < VIEW_SIZE.y;
         y += GRID_SY)
  ...
  // Move user and hint channels to top
  {
    ...
    ImDrawList_SwapChannels(m_DrawList, c_UserChannel_HintsBackground,
                            channelCount + 0);
    ImDrawList_SwapChannels(m_DrawList, c_UserChannel_Hints,
                            channelCount + 1);
  ...
  // #debug
  // drawList->AddRectFilled(ImVec2(-10.0f, -10.0f),
  // ImVec2(10.0f, 10.0f), IM_COL32(255, 0, 255, 255));
  ...
  if (!m_CurrentAction && m_IsFirstFrame &&
      !m_Settings.m_Selection.empty()) {
    ClearSelection();
...
bool ed::EditorContext::DoLink(LinkId id, PinId startPinId,
                               PinId endPinId, ImU32 color,
                               float thickness)
{
  // auto& editorStyle = GetStyle();
  ...
  if (!startPin || !startPin->m_IsLive || !endPin ||
      !endPin->m_IsLive)
    return false;
...
void ed::EditorContext::SetNodePosition(NodeId nodeId,
...
void ed::EditorContext::SetGroupSize(NodeId nodeId,
                                     const ImVec2 &size)
{
  auto node = FindNode(nodeId);
  if (!node) {
...
void ed::EditorContext::UpdateNodeState(Node *node)
{
  ...
  if (tryLoadState) {
    ...
    if (NodeSettings::Parse(m_Config.LoadNode(node->m_ID),
                            newSettings))
  ...
  node->m_GroupBounds.Max =
      node->m_GroupBounds.Min + settings->m_GroupSize;
  node->m_GroupBounds.Floor();
}
...
{
  auto objectIt = std::find(m_SelectedObjects.begin(),
                            m_SelectedObjects.end(), object);
...
bool ed::EditorContext::IsSelected(Object *object)
{
  ...
  // m_SelectedObjects.end(), object) != m_SelectedObjects.end();
}
...
void ed::EditorContext::FindLinksInRect(const ImRect &r,
                                        vector<Link *> &result,
                                        bool append)
{
  if (!append)
...
bool ed::EditorContext::HasAnyLinks(PinId pinId) const
{
  for (auto link : m_Links) {
    ...
    if (link->m_StartPin->m_ID == pinId ||
        link->m_EndPin->m_ID == pinId)
      return true;
  }
...
void ed::EditorContext::FindLinksForNode(NodeId nodeId,
                                         vector<Link *> &result,
                                         bool add)
{
  if (!add)
...
void ed::EditorContext::Suspend(SuspendFlags flags)
{
  ...
  if ((flags & SuspendFlags::KeepSplitter) !=
      SuspendFlags::KeepSplitter)
    ImDrawList_SwapSplitter(m_DrawList, m_Splitter);
}
...
ImU32 ed::EditorContext::GetColor(StyleColor colorIndex,
                                  float alpha) const
{
...
{
  auto it = std::find(m_LiveAnimations.begin(),
                      m_LiveAnimations.end(), animation);
...
void ed::EditorContext::SetUserContext(bool globalSpace)
{
  ...
  // #debug
  // drawList->AddCircleFilled(ImGui::GetMousePos(), 4, IM_COL32(0,
  // 255, 0, 255));
}
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen)
{
  ...
  auto isMouseOffscreen =
      allowOffscreen && !editorRect.Contains(mousePos);
  if (isMouseOffscreen) {
  ...
  static auto invisibleButtonEx =
      [](const char *str_id, const ImVec2 &size_arg,
    ...
    auto buttonIndex =
        ImGui::GetCurrentContext()->ActiveIdMouseButton;

    bool hovered, held;
    bool pressed =
        ButtonBehavior(bb, id, &hovered, &held, extraFlags);

    return pressed ? buttonIndex : -1;
  };

  // Emits invisible button and returns true if it is clicked.
  auto emitInteractiveAreaEx =
      [&activeId](ObjectId id, const ImRect &rect,
                  ImGuiButtonFlags extraFlags) -> int {
    ...
    // debug
    // if (id < 0) return ImGui::Button(idString,
    // to_imvec(rect.size));
  ...
  auto checkInteractionsInArea =
      [this, &emitInteractiveArea, &hotObject, &activeObject,
    ...
    if (node->m_Type == NodeType::Group) {
      ...
      for (auto region : c_Regions) {
        ...
        if (ImRect_IsEmpty(bounds))
          continue;
        checkInteractionsInArea(NodeId(static_cast<int>(region)),
                                bounds, node);
      }
  ...
  if (m_Config.DragButtonIndex == 0 ||
      m_Config.SelectButtonIndex == 0 ||
      m_Config.NavigateButtonIndex == 0)
    backgroundExtraFlags |= ImGuiButtonFlags_MouseButtonLeft;
  if (m_Config.DragButtonIndex == 1 ||
      m_Config.SelectButtonIndex == 1 ||
      m_Config.NavigateButtonIndex == 1)
    backgroundExtraFlags |= ImGuiButtonFlags_MouseButtonRight;
  if (m_Config.DragButtonIndex == 2 ||
      m_Config.SelectButtonIndex == 2 ||
      m_Config.NavigateButtonIndex == 2)
    backgroundExtraFlags |= ImGuiButtonFlags_MouseButtonMiddle;

  auto isMouseDoubleClickOverBackground =
      [doubleClickedObject, backgroundExtraFlags]() -> int {
    if (doubleClickedObject)
      return -1;
  ...
  auto backgroundClickButonIndex = emitInteractiveAreaEx(
      NodeId(0), editorRect, backgroundExtraFlags);
  auto backgroundDoubleClickButtonIndex =
      isMouseDoubleClickOverBackground();
  ...
  if (!isDragging && isBackgroundActive && hotLink &&
      !m_LastActiveLink)
  ...
  if (!isDragging && backgroundDoubleClickButtonIndex >= 0 &&
      hotLink) {
  ...
  if (ImGui::IsAnyItemActive() &&
      ImGui::GetActiveID() != m_EditorActiveId)
    return Control();
  ...
  return Control(hotObject, activeObject, clickedObject,
                 doubleClickedObject, isBackgroundHot,
...
void ed::EditorContext::ShowMetrics(const Control &control)
{
  ...
  ImGui::Text("Accept Input: %s",
              CanAcceptUserInput() ? "true" : "false");
  ImGui::Text("View Position: { x=%g y=%g }", viewRect.Min.x,
              viewRect.Min.y);
  ...
  ImGui::Text("Mouse: { x=%.0f y=%.0f } global: { x=%g y=%g }",
              localMousePos.x, localMousePos.y, globalMousePos.x,
              globalMousePos.y);
  ...
  ImGui::Text("Action: %s", m_CurrentAction
                                ? m_CurrentAction->GetName()
                                : "<none>");
...
bool ed::NodeSettings::Parse(const json::value &data,
                             NodeSettings &result)
{
  if (!data.is_object())
    return false;

  auto tryParseVector = [](const json::value &v,
                           ImVec2 &result) -> bool {
    if (v.is_object()) {
      auto xValue = v["x"];
      auto yValue = v["y"];
...
bool ed::Settings::Parse(const std::string &string,
                         Settings &settings)
{
  ...
  auto deserializeObjectId = [](const std::string &str) {
    ...
    auto id =
        reinterpret_cast<void *>(strtoull(idStart, nullptr, 10));
...
void ed::NavigateAnimation::NavigateTo(const ImRect &target,
                                       float duration)
{
  Stop();
  ...
  auto epsilon = 1e-4f;
  if (ImFabs(minoffset.x) < epsilon &&
      ImFabs(minoffset.y) < epsilon &&
      ImFabs(maxOffset.x) < epsilon &&
...
ed::FlowAnimation::FlowAnimation(FlowAnimationController *controller)
    : Animation(controller->Editor), Controller(controller),
      m_Link(nullptr), m_Offset(0.0f), m_PathLength(0.0f)
{
}

void ed::FlowAnimation::Flow(ed::Link *link, float markerDistance,
...
void ed::FlowAnimation::Draw(ImDrawList *drawList)
{
  ...
  if (IsPathValid()) {
    ...
    for (float d = m_Offset; d < m_PathLength; d += m_MarkerDistance)
      drawList->AddCircleFilled(SamplePath(d), markerRadius,
                                markerColor);
  }
}
...
void ed::FlowAnimation::UpdatePath()
{
  ...
  m_PathLength =
      ImCubicBezierLength(curve.P0, curve.P1, curve.P2, curve.P3);

  auto collectPointsCallback =
      [this](ImCubicBezierFixedStepSample &result) {
  ...
  m_Path.resize(0);
  ImCubicBezierFixedStep(collectPointsCallback, curve, step, false,
                         0.5f, 0.001f);
}
...
ed::FlowAnimationController::GetOrCreate(Link *link)
{
  ...
  // There are no live animations for target link, try to reuse
  // inactive old one
  if (!m_FreePool.empty()) {
...
ed::NavigateAction::NavigateAction(EditorContext *editor,
    ...
    : EditorAction(editor), m_IsActive(false), m_Zoom(1),
      m_VisibleRect(), m_Scroll(0, 0), m_ScrollStart(0, 0),
      m_ScrollDelta(0, 0), m_Canvas(canvas), m_WindowScreenPos(0, 0),
      m_WindowScreenSize(0, 0), m_Animation(editor, *this),
      m_Reason(NavigationReason::Unknown), m_LastSelectionId(0),
      m_LastObject(nullptr), m_MovingOverEdge(false),
...
ed::NavigateAction::Accept(const Control &control)
{
  ...
  if (Editor->CanAcceptUserInput() /*&& !ImGui::IsAnyItemActive()*/ &&
      ImGui::IsMouseDragging(Editor->GetConfig().NavigateButtonIndex,
                             0.0f)) {
    m_IsActive = true;
  ...
  // // #debug
  // if (m_DrawList)
  //     m_DrawList->AddCircleFilled(io.MousePos, 4.0f, IM_COL32(255,
  //     0, 255, 255));
...
bool ed::NavigateAction::Process(const Control &control)
{
  ...
  if (ImGui::IsMouseDragging(Editor->GetConfig().NavigateButtonIndex,
                             0.0f)) {
    m_ScrollDelta = ImGui::GetMouseDragDelta(
        Editor->GetConfig().NavigateButtonIndex);
    m_Scroll = m_ScrollStart - m_ScrollDelta * m_Zoom;
...
bool ed::NavigateAction::HandleZoom(const Control &control)
{
  ...
  const auto allowOffscreen =
      currentAction && currentAction->IsDragging();
  ...
  auto newZoom = MatchZoom(
      steps, m_ZoomLevels[steps < 0 ? 0 : m_ZoomLevelCount - 1]);
  ...
  NavigateTo(targetRect, c_MouseZoomDuration,
             NavigationReason::MouseZoom);
...
void ed::NavigateAction::NavigateTo(const ImRect &bounds,
                                    ZoomMode zoomMode, float duration,
...
void ed::NavigateAction::NavigateTo(const ImRect &target,
                                    float duration,
                                    NavigationReason reason)
{
  m_Reason = reason;
...
bool ed::NavigateAction::MoveOverEdge(const ImVec2 &canvasSize)
{
  ...
  const auto offset =
      ImMin(ImMax(screenPointOnEdge - screenMousePos, minDistance),
            maxDistance);
  const auto relativeOffset =
      -offset * io.DeltaTime * c_MaxMoveOverEdgeSpeed;
...
ed::SizeAction::Accept(const Control &control)
{
  ...
  if (control.ActiveNode && IsGroup(control.ActiveNode) &&
      ImGui::IsMouseDragging(Editor->GetConfig().DragButtonIndex,
                             1)) {
...
bool ed::SizeAction::Process(const Control &control)
{
  if (m_Clean) {
    ...
    if (m_SizedNode->m_Bounds.Min != m_StartBounds.Min ||
      ...
      Editor->MakeDirty(SaveReasonFlags::Position |
...
void ed::SizeAction::ShowMetrics()
{
  ...
  if (m_SizedNode && m_IsActive) {
    ...
    ImGui::Text("    Minimum Size: { w=%g h=%g }", m_MinimumSize.x,
                m_MinimumSize.y);
    ImGui::Text("    Last Size: { w=%g h=%g }", m_LastSize.x,
                m_LastSize.y);
  }
}
...
bool ed::DragAction::Process(const Control &control)
{
  ...
  if (control.ActiveObject == m_DraggedObject) {
    auto dragOffset = ImGui::GetMouseDragDelta(
        Editor->GetConfig().DragButtonIndex, 0.0f);
    ...
    auto alignedOffset =
        Editor->AlignPointToGrid(draggedOrigin + dragOffset +
                                 alignPivot) -
...
void ed::DragAction::ShowMetrics()
{
  ...
  ImGui::Text("    Node: %s (%p)", getObjectName(m_DraggedObject),
              m_DraggedObject ? m_DraggedObject->ID().AsPointer()
                              : nullptr);
}
...
ed::SelectAction::SelectAction(EditorContext *editor)
    : EditorAction(editor), m_IsActive(false), m_SelectGroups(false),
      m_SelectLinkMode(false), m_CommitSelection(false),
...
ed::SelectAction::Accept(const Control &control)
{
  ...
  if (Editor->CanAcceptUserInput() && control.BackgroundHot &&
      ImGui::IsMouseDragging(Editor->GetConfig().SelectButtonIndex,
                             1)) {
    m_IsActive = true;
    m_StartPoint =
...
bool ed::SelectAction::Process(const Control &control)
{
  ...
  if (ImGui::IsMouseDragging(Editor->GetConfig().SelectButtonIndex,
                             0)) {
    ...
    m_CandidateObjects.erase(std::unique(m_CandidateObjects.begin(),
...
void ed::SelectAction::Draw(ImDrawList *drawList)
{
  ...
  const auto alpha = m_Animation.IsPlaying()
                         ? ImEasing::EaseOutQuad(
                               1.0f, -1.0f, m_Animation.GetProgress())
                         : 1.0f;

  const auto fillColor =
      Editor->GetColor(m_SelectLinkMode ? StyleColor_LinkSelRect
...
ed::ContextMenuAction::Accept(const Control &control)
{
  ...
  const auto isDragging = ImGui::IsMouseDragging(
      Editor->GetConfig().ContextMenuButtonIndex, 1);
  ...
  if (candidateAction != None) {
    if (candidateAction != Paste && candidateAction != CreateNode) {
      ...
      if (!selection.empty()) {
        ...
        if (!extra.empty()) {
          m_Context.insert(m_Context.end(), extra.begin(),
                           extra.end());
          ...
          m_Context.erase(
              ...
              m_Context.end());
                 ...
                 control.HotObject->IsSelectable() &&
      ...
      if (!hasAnyLinks &&
          m_Context.size() >
              1) // one node cannot make connection to anything
      {
        ...
        // Collect links connected to nodes and drop those reaching
        // out of context
...
ed::CreateItemAction::CreateItemAction(EditorContext *editor)
    : EditorAction(editor), m_InActive(false), m_NextStage(None),
      ...
      m_LinkColor(IM_COL32_WHITE), m_LinkThickness(1.0f),
      m_LinkStart(nullptr), m_LinkEnd(nullptr),
...
bool ed::CreateItemAction::Process(const Control &control)
{
  ...
  if (m_DraggedPin && control.ActivePin == m_DraggedPin &&
    ...
    ed::Pin cursorPin(Editor, 0,
                      draggingFromSource ? PinKind::Input
                                         : PinKind::Output);
    cursorPin.m_Pivot =
    ...
    candidate.m_StartPin =
        draggingFromSource ? m_DraggedPin : &cursorPin;
    candidate.m_EndPin =
        draggingFromSource ? &cursorPin : m_DraggedPin;

    ed::Pin *&freePin = draggingFromSource ? candidate.m_EndPin
                                           : candidate.m_StartPin;
...
ed::CreateItemAction::QueryLink(PinId *startId, PinId *endId)
{
  ...
  *endId = linkEndId;
  ...
  if (!m_IsInGlobalSpace) {
    ...
    ImGui::PushClipRect(rect.Min + ImVec2(1, 1),
                        rect.Max - ImVec2(1, 1), false);
...
void ed::DeleteItemsAction::DeleteDeadLinks(NodeId nodeId)
{
  ...
  for (auto link : links) {
    ...
    auto it = std::find(m_CandidateObjects.begin(),
                        m_CandidateObjects.end(), link);
...
void ed::DeleteItemsAction::ShowMetrics()
{
  ...
  ImGui::Text("    Active: %s", m_IsActive ? "yes" : "no");
  // ImGui::Text("    Node: %s (%d)",
  // getObjectName(DeleteItemsgedNode), DeleteItemsgedNode ?
...
bool ed::DeleteItemsAction::QueryItem(ObjectId *itemId,
...
{
  auto item = m_CandidateObjects[m_CandidateItemIndex];
  m_CandidateObjects.erase(m_CandidateObjects.begin() +
                           m_CandidateItemIndex);
...
void ed::NodeBuilder::Begin(NodeId nodeId)
{
  ...
  if (m_CurrentNode->m_CenterOnScreen) {
    ...
    auto offset =
        bounds.GetCenter() - m_CurrentNode->m_Bounds.GetCenter();
  ...
  m_CurrentNode->m_GroupColor =
      Editor->GetColor(StyleColor_GroupBg, alpha);
...
void ed::NodeBuilder::End()
{
  ...
  // Apply frame padding. This must be done in this convoluted way if
  // outer group size must contain inner group padding.
  auto &editorStyle = Editor->GetStyle();
  if (editorStyle.NodePadding.x != 0 ||
      editorStyle.NodePadding.y != 0 ||
      editorStyle.NodePadding.z != 0 ||
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing,
                        ImVec2(0.0f, 0.0f));
    ImGui::Dummy(ImVec2(0, 0)); // bump cursor at the end of the line
                                // and move to next one
    ImGui::Dummy(
        ImVec2(0, editorStyle.NodePadding.w)); // apply padding
    ImGui::PopStyleVar();
  }
  ...
  if (m_IsGroup) {
    ...
    for (auto pin = m_CurrentNode->m_LastPin; pin;
         pin = pin->m_PreviousPin)
      pin->Reset();
...
void ed::NodeBuilder::BeginPin(PinId pinId, PinKind kind)
{
  ...
  m_CurrentPin->m_BorderColor =
      Editor->GetColor(StyleColor_PinRectBorder);
...
ed::NodeBuilder::GetUserBackgroundDrawList(Node *node) const
{
  if (node && node->m_IsLive) {
    auto drawList = Editor->GetDrawList();
    drawList->ChannelsSetCurrent(node->m_Channel +
                                 c_NodeUserBackgroundChannel);
    return drawList;
  } else
    return nullptr;
}
...
bool ed::HintBuilder::Begin(NodeId nodeId)
{
  ...
  const auto alpha =
      ImMax(0.0f, std::min(1.0f, (view.Scale - c_min_zoom) /
                                     (c_max_zoom - c_min_zoom)));

  Editor->GetDrawList()->ChannelsSetCurrent(
...
std::string ed::Config::LoadNode(NodeId nodeId)
{
  ...
  if (LoadNodeSettings) {
    ...
    if (size > 0) {
      data.resize(size);
      LoadNodeSettings(nodeId, const_cast<char *>(data.data()),
                       UserPointer);
    }
  }
...
{
  if (SaveSettings) {
    return SaveSettings(data.c_str(), data.size(), flags,
                        UserPointer);
  } else if (SettingsFile) {
