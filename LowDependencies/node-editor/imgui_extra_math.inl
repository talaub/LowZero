                                  ...
                                  bool snap_to_edge)
{
  ...
  return ImVec2(
      (p.x > rect.Max.x) ? rect.Max.x
                         : (p.x < rect.Min.x ? rect.Min.x : p.x),
                                  ...
                                  bool snap_to_edge, float radius)
{
  ...
  return point +
         offset * (ImMin(distance, radius) * (1.0f / distance));
}

inline ImVec2 ImRect_ClosestPoint(const ImRect &rect,
                                  const ImRect &other)
{
  ImVec2 result;
  if (other.Min.x >= rect.Max.x)
    ...
    result.x = (ImMax(rect.Min.x, other.Min.x) +
                ImMin(rect.Max.x, other.Max.x)) /
               2;
  ...
  if (other.Min.y >= rect.Max.y)
    ...
    result.y = (ImMax(rect.Min.y, other.Min.y) +
                ImMin(rect.Max.y, other.Max.y)) /
               2;

  return result;
}

inline ImLine ImRect_ClosestLine(const ImRect &rect_a,
                                 const ImRect &rect_b)
{
  ImLine result;
  ...
  auto distribute = [](float &a, float &b, float a0, float a1,
                       float b0, float b1) {
    if (a0 >= b1 || a1 <= b0)
      return;
  ...
  distribute(result.A.x, result.B.x, rect_a.Min.x, rect_a.Max.x,
             rect_b.Min.x, rect_b.Max.x);
  distribute(result.A.y, result.B.y, rect_a.Min.y, rect_a.Max.y,
             rect_b.Min.y, rect_b.Max.y);

  return result;
}

inline ImLine ImRect_ClosestLine(const ImRect &rect_a,
                                 const ImRect &rect_b, float radius_a,
                                 float radius_b)
{
