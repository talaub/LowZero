...
namespace Low {
  namespace Renderer {
    namespace Backend {
      const uint16_t GraphicsPipeline::TYPE_ID = 1;
      ...
      void
      GraphicsPipeline::set_pipeline(Low::Renderer::Backend::Pipeline &p_Value)
      {
        _LOW_ASSERT(is_alive());

        // Set new value
        TYPE_SOA(GraphicsPipeline, pipeline, Low::Renderer::Backend::Pipeline) =
            p_Value;
