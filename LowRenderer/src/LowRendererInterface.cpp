  ...
  namespace Renderer {
    namespace Interface {
      namespace PipelineManager {
        ...
        static void
        recreate_compute_pipeline(ComputePipeline p_Pipeline,
                                  bool p_CleanOld = true);
        static void
        recreate_graphics_pipeline(GraphicsPipeline p_Pipeline,
                                   bool p_CleanOld = true);
        ...
        Util::Map<ComputePipeline, Util::String>
            g_ComputePipelineOutPaths;

        Util::Map<Util::String, Util::List<GraphicsPipeline>>
            g_GraphicsPipelines;
        Util::Map<Util::String, Util::List<ComputePipeline>>
            g_ComputePipelines;
            ...
            PipelineComputeCreateParams &p_Params)
        {
          ...
          if (!l_AlreadyExisting) {
            ...
            if (g_ComputePipelines.find(l_OutputPath) ==
              ...
              g_ComputePipelines[l_OutputPath] =
                  Util::List<ComputePipeline>();
            }
        ...
        {
          if (p_CleanOld) {
            Backend::callbacks().pipeline_cleanup(
          ...
          Util::String l_OutputPath =
              g_ComputePipelineOutPaths[p_Pipeline];
          ...
          for (uint8_t i = 0; i < l_BeParams.signatureCount; ++i) {
            l_Signatures.push_back(
                l_Params.signatures[i].get_signature());
          }

          l_BeParams.signatures = l_Signatures.data();

          l_BeParams.pipelineConstantCount =
              l_Params.constants.size();
            ...
            PipelineGraphicsCreateParams &p_Params)
        {
          ...
          if (!l_AlreadyExisting) {
            ...
            l_OutputPaths.fragment =
                compile(p_Params.fragmentShaderPath);
            ...
            g_GraphicsPipelines[l_OutputPaths.vertex].push_back(
                p_Pipeline);
            g_GraphicsPipelines[l_OutputPaths.fragment].push_back(
                p_Pipeline);
                                   ...
                                   bool p_CleanOld)
        {
          ...
          l_BeParams.fragmentShaderPath =
              l_OutputPaths.fragment.c_str();
          ...
          l_BeParams.renderpass =
              &l_Params.renderpass.get_renderpass();
          ...
          l_BeParams.vertexDataAttributeCount = static_cast<uint8_t>(
              l_Params.vertexDataAttributeTypes.size());
          ...
          l_BeParams.depthCompareOperation =
        ...
        static void recreate_pipeline(Util::String p_OutPath)
        {
          ...
          for (uint32_t i = 0;
               i < g_GraphicsPipelines[p_OutPath].size(); ++i) {
            GraphicsPipeline i_Pipeline =
                g_GraphicsPipelines[p_OutPath][i];
          ...
          for (uint32_t i = 0;
               i < g_ComputePipelines[p_OutPath].size(); ++i) {
            ComputePipeline i_Pipeline =
                g_ComputePipelines[p_OutPath][i];
        ...
        static void do_tick(float p_Delta)
        {
          for (auto it = g_SourceTimes.begin();
            ...
            Util::String i_SourcePath =
                Backend::callbacks().get_shader_source_path(
                    it->first);
        ...
        void delist_graphics_pipeline(GraphicsPipeline p_Pipeline)
        {
          ...
          g_GraphicsPipelines[l_OutputPaths.vertex].erase_first(
              p_Pipeline);
          g_GraphicsPipelines[l_OutputPaths.fragment].erase_first(
              p_Pipeline);
