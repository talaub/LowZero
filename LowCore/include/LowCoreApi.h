...
/* Define LOW_CORE_API and related macros for both Windows and
   Unix-like compilers. Usage: prefix any symbol that should be
   exported from the shared library with LOW_CORE_API. Example: class
   LOW_CORE_API Foo { ... }; LOW_CORE_API void do_stuff();
*/

/* If building/using a static library, no import/export attributes are
 * needed. */
...
#define LOWCORE_DEPRECATED_NO_EXPORT                                 \
  LOWCORE_NO_EXPORT LOWCORE_DEPRECATED
#endif

/* Optionally allow projects to disable deprecated declarations by
 * defining LOWCORE_NO_DEPRECATED. */
