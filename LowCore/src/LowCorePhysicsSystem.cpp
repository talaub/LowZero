  ...
  namespace Core {
    ...
    namespace System {
      namespace Physics {
        ...
        void initialize()
        {
          g_Foundation = PxCreateFoundation(
              PX_PHYSICS_VERSION, g_Allocator, g_CustomErrorCallback);
          LOW_ASSERT(g_Foundation,
                     "Could not create physx foundation");

          g_ToleranceScale.length =
              100; // typical length of an object
          g_ToleranceScale.speed =
              981; // typical speed of an object,
          ...
          LOW_ASSERT(g_Dispatcher,
                     "Could not create physx dispachter");

          g_DefaultMaterial =
              g_Base->createMaterial(0.5f, 0.5f, 0.5f);
          LOW_ASSERT(g_DefaultMaterial,
                     "Could not create physx material");
        ...
        static Math::Color
        get_color_for_physx_color(physx::PxU32 p_Color)
        {
          using namespace physx;

          uint8_t *l_Channels = (uint8_t *)&p_Color;
          return Math::Color(
              to_float(l_Channels[0]), to_float(l_Channels[1]),
        ...
        void tick(float p_Delta, Util::EngineState p_State)
        {
          ...
          for (auto it =
                   Component::Rigidbody::ms_LivingInstances.begin();
               it != Component::Rigidbody::ms_LivingInstances.end();
               ++it) {
            Component::Transform i_Transform =
                it->get_entity().get_transform();

            Math::Matrix4x4 i_LocalMatrix(1.0f);
            i_LocalMatrix = glm::translate(
                i_LocalMatrix, it->get_shape().box.position);
            i_LocalMatrix *=
                glm::toMat4(it->get_shape().box.rotation);
            ...
            glm::decompose(i_WorldMatrix, i_WorldScale,
                           i_WorldRotation, i_WorldPosition,
                           i_WorldSkew, i_WorldPerspective);
        ...
        void late_tick(float p_Delta, Util::EngineState p_State)
        {
          ...
          if (PHYSICS_VISUALIZATION) {
            ...
            for (PxU32 i = 0; i < rb.getNbLines(); i++) {
              ...
              Math::Box l_Box;
              l_Box.position = Math::Vector3(line.pos0.x, line.pos0.y,
                                             line.pos0.z);
              l_Box.rotation =
                  Math::Quaternion(1.0f, 0.0f, 0.0f, 0.0f);
              l_Box.halfExtents = Math::Vector3(0.05f);
              DebugGeometry::render_box(
                  l_Box, get_color_for_physx_color(line.color0), true,
                  false);
            }
          }
          ...
          /*
                for (auto it =
             Component::Rigidbody::ms_LivingInstances.begin(); it !=
             Component::Rigidbody::ms_LivingInstances.end(); ++it) {
             if (it->is_fixed()) {
                    // continue;
                  }
                  ...
                  Component::Transform i_Transform =
             it->get_entity().get_transform();
                  // TODO: Hack for top level - does not work for
             transform hierarchy
        ...
        void
        register_rigid_dynamic(PhysicsRigidDynamic &p_RigidDynamic)
        {
    ...
    static PxShape *generate_pxshape(Math::Shape &p_Shape)
    {
      ...
      if (p_Shape.type == Math::ShapeType::BOX) {
        ...
        return g_Base->createShape(
            PxBoxGeometry(l_Box.halfExtents.x * 2.0f,
    ...
    void PhysicsRigidDynamic::create(
        PhysicsRigidDynamic &p_RigidDynamic, PhysicsShape &p_Shape,
        ...
        void *p_UserData)
    {
      ...
      p_RigidDynamic.m_RigidDynamic = PxCreateDynamic(
          *g_Base,
          PxTransform(
              PxVec3(p_Position.x, p_Position.y, p_Position.z),
              l_Quat),
          *(p_Shape.m_Shape), 1.0f);

      p_RigidDynamic.m_RigidDynamic->setActorFlag(
          PxActorFlag::eVISUALIZATION, PHYSICS_VISUALIZATION);
    ...
    {
      m_RigidDynamic->setActorFlag(PxActorFlag::eDISABLE_GRAVITY,
                                   !p_Gravity);
      m_RigidDynamic->wakeUp();
    }
    ...
    physx::PxTransform
    PhysicsRigidDynamic::get_physx_transform() const
    {
    ...
    Math::Vector3 PhysicsRigidDynamic::get_position() const
    {
      physx::PxTransform l_Transform =
          m_RigidDynamic->getGlobalPose();

      return Math::Vector3(l_Transform.p.x, l_Transform.p.y,
                           l_Transform.p.z);
    }
    ...
    void PhysicsRigidDynamic::update_transform(
        Math::Vector3 &p_Position, Math::Quaternion &p_Rotation)
    {
      PxVec3 l_Pos(p_Position.x, p_Position.y, p_Position.z);
      PxQuat l_Rot(p_Rotation.x, p_Rotation.y, p_Rotation.z,
                   p_Rotation.w);
    ...
    void
    PhysicsRigidDynamic::update_shape(PhysicsShape &p_PhysicsShape)
    {
    ...
    namespace Physics {
      namespace Helper {
        ...
        {
          return PxVec4(p_Vector.x, p_Vector.y, p_Vector.z,
                        p_Vector.w);
        }
        ...
        {
          return Math::Quaternion(p_Quat.w, p_Quat.x, p_Quat.y,
                                  p_Quat.z);
        }
                   ...
                   float p_MaxDistance, RaycastHit &p_Hit)
      {
        ...
        bool l_Status = g_Scene->raycast(l_Origin, l_Direction,
                                         l_MaxDistance, l_Hit);
      ...
      inline bool get_overlapping_tringle_mesh(
          const PxGeometry &p_ShapeGeometry,
          const PxTransform &p_ShapePose,
          const PxTriangleMeshGeometry &p_MeshGeometry,
          const PxTransform &p_MeshPose,
          ...
          uint16_t p_MaxTriangleVertices)
      {
        ...
        const uint32_t *__restrict l_Indices =
            l_OverlapUtil.getResults();
        ...
        for (uint32_t i = 0; i < l_TriangleCount; ++i) {
          ...
          PxMeshQuery::getTriangle(p_MeshGeometry, p_MeshPose,
                                   i_TriangleId, i_Triangle);

          for (uint8_t j = 0; j < 3u; ++j) {
            p_Vertices.push_back(
                Helper::convert(i_Triangle.verts[j]));
          }
        }
          ...
          uint16_t p_MaxTriangleVertices)
      {
        PxMeshOverlapUtil l_OverlapUtil;
        const uint32_t l_TriangleCount = l_OverlapUtil.findOverlap(
            p_ShapeGeometry, p_ShapePose, p_HeightFieldGeometry,
            p_MeshPose);
          ...
          LOW_ASSERT(
              i_TriangleId <
                  p_HeightFieldGeometry.heightField->getNbRows() *
                      p_HeightFieldGeometry.heightField
                          ->getNbColumns() *
                      2u,
              "Invalid triangle ID");
          ...
          bool p_IgnoreTriggers /*= true*/)
      {
        ...
        if (p_HalfExtents.x < EPSILON || p_HalfExtents.y < EPSILON ||
          ...
          LOW_LOG_WARN << "Half extents must be positive"
                       << LOW_LOG_END;
        }
        ...
        uint32_t l_MemSize =
            sizeof(PxOverlapHit) * RESULT_BUFFER_SIZE;

        // PERF: Replace with scratch allocator!!
        void *l_Memory =
            Util::Memory::main_allocator()->allocate(l_MemSize);
        PxOverlapHit *l_Buffer =
            static_cast<PxOverlapHit *>(l_Memory);
        ...
        if (l_Hit && l_HitBuffer.hasAnyHits()) {
          ...
          for (uint32_t i = 0; i < l_TouchCount; ++i) {
            Helper::get_hit_info(l_HitBuffer.getTouch(i),
                                 p_Touches[i]);
          }
        }
          ...
          bool p_IgnoreTriggers /*= true*/)
      {
        ...
        overlap_box(l_Scene, p_HalfExtents, p_Position, p_Orientation,
                    l_OverlappingActors, p_CollisionMask,
                    p_SceneQueryFlags, p_MaxMeshOverlaps,
                    p_IgnoreTriggers);

        for (const auto &i_Overlap : l_OverlappingActors) {
          if (p_Vertices.size() < p_MaxTriangleVertices &&
              i_Overlap.shape &&
              i_Overlap.shape->getConcreteType() ==
            ...
            if (i_Overlap.shape->getGeometryType() ==
              ...
              get_overlapping_tringle_mesh(
                  ...
                  PxTransform(Helper::convert(p_Position),
                              Helper::convert(p_Orientation)),
                  triangleMesh, i_Overlap.actor->getGlobalPose(),
                  p_Vertices,
              ...
              get_overlapping_heightfield(
                  ...
                  PxTransform(Helper::convert(p_Position),
                              Helper::convert(p_Orientation)),
                  i_HeightField, i_Overlap.actor->getGlobalPose(),
                  p_Vertices,
                  p_MaxTriangleVertices - p_Vertices.size());
            } else {
          ...
          if (p_Vertices.size() == p_MaxTriangleVertices) {
            LOW_LOG_WARN
                << "Maximum number of triangles retrieved with an "
                   "overlap query ("
